...
class RandomBDSPTeams {
  ...
  shouldCullMove(move, types, moves, abilities, counter, movePool, teamDetails, species, isLead) {
    ...
    switch (move.id) {
      ...
      case "extremespeed":
      case "rockblast":
        return { cull: !!counter.get("speedsetup") || moves.has("dragondance") || counter.damagingMoves.size < 2 };
      case "closecombat":
      case "flashcannon":
      ...
      case "protect":
        ...
        if (counter.get("Status") < 2 && ["Guts", "Quick Feet", "Speed Boost", "Moody"].every((m) => !abilities.includes(m))) return { cull: true };
        if (movePool.includes("leechseed") || movePool.includes("toxic") && !moves.has("wish")) return { cull: true };
        if (["bellydrum", "fakeout", "shellsmash", "spore"].some((m) => movePool.includes(m)) || moves.has("tailwind") || moves.has("waterspout")) return { cull: true };
        return { cull: false };
      case "rapidspin":
      ...
      case "thunderwave":
      case "voltswitch":
        return { cull: !!counter.setupType || !!counter.get("speedsetup") || moves.has("shiftgear") || moves.has("raindance") };
      case "toxic":
      ...
      case "uturn":
        ...
        return { cull: !!counter.get("speedsetup") || counter.setupType && !bugSwordsDanceCase || abilities.includes("Speed Boost") && moves.has("protect") };
      /**
      ...
      case "leafstorm":
        ...
        return {
          cull: counter.setupType === "Physical" && leafBladePossible || moves.has("gigadrain") && !!counter.get("Status")
        };
      case "freezedry":
      ...
      case "leechlife":
        return {
          cull: moves.has("uturn") && !counter.setupType || movePool.includes("spikes")
        };
      case "stoneedge":
  ...
  shouldCullAbility(ability, types, moves, abilities, counter, movePool, teamDetails, species) {
    ...
    switch (ability) {
      ...
      case "Unburden":
        ...
        abilities.includes("Reckless") || !counter.setupType;
      case "Volt Absorb":
  ...
  getMediumPriorityItem(ability, moves, counter, species, isLead) {
    if (moves.size === 1) {
    ...
    if ((counter.get("Physical") >= 3 && moves.has("defog") || counter.get("Special") >= 3 && moves.has("healingwish")) && !counter.get("priority") && !moves.has("uturn")) return "Choice Scarf";
    if (moves.has("raindance") || moves.has("sunnyday") || ability === "Speed Boost" && !counter.get("hazards")) return "Life Orb";
    if (["clearsmog", "curse", "haze", "healbell", "protect", "sleeptalk"].some((m) => moves.has(m)) && ability === "Moody") return "Leftovers";
  }
  getLowPriorityItem(ability, types, moves, abilities, counter, teamDetails, species, isLead) {
    const defensiveStatTotal = species.baseStats.hp + species.baseStats.def + species.baseStats.spd;
    if (isLead && ability !== "Sturdy" && !moves.has("substitute") && !counter.get("drain") && !counter.get("recoil") && !counter.get("recovery") && (defensiveStatTotal <= 250 && counter.get("hazards") || defensiveStatTotal <= 210)) return "Focus Sash";
    ...
    counter.get("drain") && moves.has("roost") || moves.has("trickroom") || moves.has("psystrike") || species.baseStats.spe > 40 && defensiveStatTotal < 275)) return "Life Orb";
    if (counter.damagingMoves.size >= 4 && !counter.get("Dragon") && !counter.get("Normal")) {
      return "Expert Belt";
    }
    if (!moves.has("substitute") && (moves.has("dragondance") || moves.has("swordsdance")) && (moves.has("outrage") || ["Bug", "Fire", "Ground", "Normal", "Poison"].every((type) => !types.has(type)) && ability !== "Storm Drain")) return "Lum Berry";
  }
  getLevel(species) {
  ...
  randomSet(species, teamDetails = {}, isLead = false) {
    ...
    do {
      ...
      for (const moveid of moves) {
        ...
        if (moveIsRejectable && (!cull && !isSetup && !move.weather && !move.stallingMove && notImportantSetup && !move.damage && this.unrejectableMovesInSingles(move))) {
          if (
