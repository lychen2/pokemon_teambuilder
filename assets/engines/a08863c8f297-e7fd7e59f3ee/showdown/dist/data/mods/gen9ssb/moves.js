  ...
  trivialpursuit: {
    ...
    beforeTurnCallback(pokemon) {
      pokemon.addVolatile(
        "trivialpursuit",
        pokemon,
        this.dex.getActiveMove("trivialpursuit")
      );
    },
    ...
    condition: {
      ...
      onFoeBeforeSwitchOut(pokemon) {
        ...
        if (!source.isAdjacent(pokemon) || !source.hp || source.volatiles["encore"] && source.volatiles["encore"].move !== "trivialpursuit" || !this.queue.cancelMove(source)) return;
  ...
  attackofopportunity: {
    ...
    beforeTurnCallback(pokemon) {
      pokemon.addVolatile(
        "attackofopportunity",
        pokemon,
        this.dex.getActiveMove("attackofopportunity")
      );
    },
    onModifyMove(move, source, target) {
      if (target?.beingCalledBack || target?.switchFlag) {
    ...
    condition: {
      ...
      onFoeBeforeSwitchOut(pokemon) {
        ...
        if (!source.isAdjacent(pokemon) || !source.hp || source.volatiles["encore"] && source.volatiles["encore"].move !== "attackofopportunity" || !this.queue.cancelMove(source)) return;
        if (source.canMegaEvo || source.canUltraBurst || source.canTerastallize) {
          for (const [actionIndex, action] of this.queue.entries()) {
            if (action.pokemon === source) {
              if (action.choice === "megaEvo") {
