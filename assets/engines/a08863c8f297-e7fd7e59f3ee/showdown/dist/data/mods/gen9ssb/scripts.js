...
for (const row of usergroupData) {
  if (!(0, import_dex_data.toID)(row)) continue;
...
function changeSet(context, pokemon, newSet, changeAbility = false) {
  ...
  if (changeAbility) pokemon.setAbility(newSet.ability, void 0, void 0, true);
  ...
  actions: {
    ...
    hitStepTryImmunity(targets, pokemon, move) {
      ...
      for (const [i, target] of targets.entries()) {
        ...
        (pokemon.hasAbility("prankster") || pokemon.hasAbility("youkaiofthedusk") || pokemon.volatiles["irpachuza"] || pokemon.hasAbility("neverendingfhunt")) && !targets[i].isAlly(pokemon) && !this.dex.getImmunity("prankster", target)) {
      ...
      switch (move.target) {
        ...
        default:
          ...
          if (this.battle.activePerHalf > 1 && !move.tracksTarget) {
            const isCharging = move.flags["charge"] && !this.volatiles["twoturnmove"] && !(move.id.startsWith("solarb") && ["sunnyday", "desolateland"].includes(this.effectiveWeather(move))) && !(move.id === "fruitfullongbow" && ["sunnyday", "desolateland"].includes(this.effectiveWeather(move))) && !(move.id === "praisethemoon" && this.battle.field.getPseudoWeather("gravity")) && !(move.id === "electroshot" && ["stormsurge", "raindance", "primordialsea"].includes(this.effectiveWeather(move))) && !(this.hasItem("powerherb") && move.id !== "skydrop");
            if (!isCharging) {
