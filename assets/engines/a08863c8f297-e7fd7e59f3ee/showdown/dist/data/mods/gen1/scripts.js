...
const Scripts = {
  ...
  pokemon: {
    ...
    deductPP(move, amount) {
      ...
      if (ppData.pp < 0) {
        this.battle.hint(
          "In Gen 1, if a Pok\xE9mon is forced to use a move with 0 PP, the move will underflow to have 63 PP.",
          void 0,
          this.side
        );
      }
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (move.id === "cannotmove") {
        if (pokemon.status === "slp") {
          this.battle.hint(
            "In Gen 1, if a Pok\xE9mon spends a turn partially trapped and switches to a Pok\xE9mon that is asleep, the sleep counter will not decrease until you select a move with a different Pok\xE9mon."
          );
        } else if (pokemon.getLockedMove()) {
          this.battle.hint(
            "In Gen 1, when Haze cures the sleep/freeze status of a Pok\xE9mon during a multi-turn move, that Pok\xE9mon will become soft-locked."
          );
        } else if (pokemon.getSemiLockedMove()) {
          this.battle.hint(
            "In Gen 1, when Haze cures the sleep/freeze status of a Pok\xE9mon during Bide, the move execution will never resolve."
          );
        }
        abortMove();
        return;
      ...
      if (move.id !== "struggle") {
        ...
        if (!lockedMove && move.id !== pokemon.side.lastEnemySelectedMove) {
          ...
          this.battle.hint(
            "In Gen 1, if both players would see the same Pok\xE9mon using different moves, the Pokemon defaults to the move shown from the perspective of the player controlling that Pok\xE9mon."
          );
        }
      }
      if (move.id === "nomove") {
        this.battle.hint(
          "In Gen 1, if a Pok\xE9mon is thawed from freeze and no move has been selected yet, the Pok\xE9mon will use a move with Fissure's animation, 102 base power, ??? type, Special category, and around 31.6% accuracy."
        );
