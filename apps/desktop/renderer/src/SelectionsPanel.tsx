...
export function SelectionsPanel({workspace: w}: {workspace: Workspace}) {
  ...
  const memberLabel = (id: string, megaId?: string | null) => {
    const member = result?.lineup.find(member => member.id === id);
    return member ? <PokemonLabel key={id} id={member.speciesId} gender={member.gender} itemId={member.itemId} mega={id === megaId}/> : <span key={id}>成员未找到</span>;
  };
  const save = async (route: SelectionRoute, lead?: SelectionRoute['leadOptions'][number]) => {
    try {
      if (!fresh || !result?.routes.some(candidate => candidate.id === route.id)) throw new Error('队伍、规则、样本或对手已改变，请等待新的选出分析。');
      const chosenLead = lead ?? route.leadOptions.find(option => option.members.length === route.leads.length && option.members.every(id => route.leads.includes(id)));
      const entry: MatchupPlan = {id: crypto.randomUUID(), revision: 0, kind: 'plan', draftId: draft.id, environmentId: draft.environmentId, snapshot: structuredClone(draft), updatedAt: new Date().toISOString(), title: `${route.title} · 待验证`, opponentSpecies,
        routes: [{id: crypto.randomUUID(), name: route.title, selection: route.members, leads: chosenLead?.members ?? route.leads, winCondition: route.winConditions.join('\n'), concerns: [...route.requirements, `首发场地条件：${fieldSummary(chosenLead?.field ?? route.field)}`, `首发 Mega：${(chosenLead ? chosenLead.megaId : route.megaId) ? nameOf((chosenLead ? chosenLead.megaId : route.megaId)!) : '不使用'}`, ...(chosenLead?.benefits ?? []), ...(chosenLead?.concerns ?? []), '由机制分析生成，尚未实战确认'].join('\n'), status: 'idea'}]};
  ...
  return <section className="research-panel"><div><h3>让每种 {w.state!.environment.pickedTeamSize} 人选出都有执行路线</h3><p className="muted">比较支援依赖、困难对手与资源条件。排序是快速线索，不能代替回合验证。</p></div>
    ...
    {result && <><p className="field-help">已枚举 {result.routes.length} 条选出与 Mega 路线。{fresh ? '' : '当前显示旧结果，正在更新。'}</p>{result.routes.map((route, i) => <details className="selection-route" key={route.id} open={i === 0}><summary><span>{String(i + 1).padStart(2, '0')} · {route.title}</span><span className="route-lineup">{route.members.map(id => {const member = result.lineup.find(m => m.id === id); return member && <PokemonIcon key={id} id={member.speciesId} gender={member.gender} itemId={member.itemId} mega={id === route.megaId} size={35}/>;})}</span></summary>
      <p className="field-help">{route.megaId ? `${nameOf(route.megaId)}使用 Mega` : '保留 Mega 资源'} · {fieldSummary(route.field)}</p><div className="pokemon-list" role="group" aria-label="候选首发"><span className="muted">候选首发：</span>{route.leads.map(id => memberLabel(id, route.megaId))}</div>
      <details className="lead-comparisons"><summary>比较 {route.leadOptions.length} 组首发</summary>{route.leadOptions.map(lead => <div key={lead.members.join(',')}><strong className="pokemon-list">{lead.members.map(id => memberLabel(id, lead.megaId))}</strong><p className="field-help">{lead.megaId ? `${nameOf(lead.megaId)}首发使用 Mega` : '首发不使用 Mega'} · {fieldSummary(lead.field)}</p><p className="field-help">进攻打点覆盖 {(lead.metrics.coverage * 100).toFixed(0)}% · 困难对局指标 {lead.metrics.tailRisk.toFixed(2)}</p>{lead.benefits.map(text => <p className="field-help" key={text}>{text}</p>)}{lead.concerns.map(text => <p className="field-help" key={text}>{text}</p>)}<button className="text-button" disabled={!fresh} onClick={() => void save(route, lead)}>用此首发保存计划</button></div>)}</details>{route.winConditions.map(text => <p key={text}>{text}</p>)}{route.dependencies.map((dependency, index) => <div className="route-dependency" key={index}><strong className="pokemon-list">{memberLabel(dependency.supportId, route.megaId)}<span>→</span>{memberLabel(dependency.beneficiaryId, route.megaId)}</strong><span>{dependency.reason}</span></div>)}
      {route.requirements.map(text => <p className="field-help" key={text}>{text}</p>)}<div className="pokemon-list" role="group" aria-label="仍需检查的对手"><span className="muted">仍需检查：</span>{route.difficultOpponents.map(id => <PokemonLabel key={id} id={id}/>)}</div><button className="button secondary small" disabled={!fresh} onClick={() => void save(route)}><Save size={13}/>保存为待验证计划</button>
