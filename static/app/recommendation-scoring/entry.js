...
export function buildRecommendationEntry(candidate, team, analysis, speedTiers, language, preferences, weights, datasets, focusType = "") {
  ...
  const linearScore = Object.values(breakdown).reduce((sum, value) => sum + value, 0);
  const floorPenalty = breakdown.resistance < 1 || breakdown.coverage < 1 ? 0.3 : 1;
  const score = linearScore * floorPenalty;
