...
export function researchEntrySchema(set: z.ZodType, draft: z.ZodType, field: z.ZodType) {
  ...
  return z.discriminatedUnion('kind', [
    z.object({...base, ...team, kind: z.literal('plan'), title: id, routes: z.array(z.object({id, name: id, ...selection, winCondition: z.string(), concerns: z.string(), status: z.enum(['idea', 'tested', 'rejected'])}))}),
    z.object({...base, ...team, ...selection, kind: z.literal('match'), playedAt: z.string(), result: z.enum(['win', 'loss', 'tie', 'unknown']), category: z.enum(['practice', 'ranked', 'tournament']), reasons: z.array(id), notes: z.string(), replayUrl: url, planId: id.optional()}),
