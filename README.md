...
**结阵**（jié zhèn）：结伴成阵。六只宝可梦里，先有锁定要围绕的**核心**，再把**搭档**一个个结成能打的**阵容**；名字取的就是这件事本身——不是"分高者胜出的评分器"，而是把一支队伍结起来、并且每一步都能复查的工作台。

标志把这件事画了出来：**一枚精灵球被截成六瓣**，对应六个队伍位置。

- 上面三瓣与正下方一瓣已经落定 —— 四只已经定下的成员（Champions 双打正是六选四）；
- 左右下两瓣只留描边 —— 还没补齐的位置，草稿状态是正常的；
- **正上方那一瓣是绿色** —— 你锁定、其余五只围绕它构筑的核心；
- 加粗的赤道带与中心按钮保留精灵球读法 —— 一眼看出这是宝可梦工具，而不是又一个待办清单。
