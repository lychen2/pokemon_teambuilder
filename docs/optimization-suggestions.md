...
> 审阅时间：2026/04/19。
>
> 实现进度更新：2026/04/20（二期核心回写：#37、#44、#46、#57、#68 已落地，详见对应条目现状段）。一期已落地 22 项：#1、#2、#3、#4、#5、#8、#11、#12、#16、#18、#20、#22、#23、#29、#31、#32、#34、#35、#36、#39、#40、#64；#14、#66、#75 仍为"部分完成"。另按用户反馈撤除了一批战况 UI toggle，详见 #1/#3/#4/#5 的"当前现状"段）。
>
...
### 9. 进攻覆盖只基于"已配招式"，忽略潜在学习招式（有可能会有不适合的招式，还得基于使用率）

**现状定位**：`analysis.js:187` 的 `summarizeOffensivePairs` 用 `team.flatMap((config) => config.offensiveTypes || [])`，而 `offensiveTypes` 在 `showdown.js:231` 只取自 `moves.filter(cat!=="Status").map(move.type)`。

**为什么是问题**：在组队阶段很多精灵的招式还没定死，用户可能只写了 3 招，或者"这只其实可以换 Ice Beam 补覆盖"。但工具只按已填招式判断盲点，就会给出"这只队伍缺冰招覆盖"的结论，尽管队里 2 只都能学 Ice Beam。

**建议做法**：引入 `getPotentialCoverage(team, datasets)` 的第二条分析线：对队伍里每只，扫一遍 `datasets.learnsets`，统计"如果换一招能补什么盲点"。UI 上用两档色：红色 = 队伍真的无招覆盖；橙色 = 有人能学但没带。点击进一步列出候选招式与替换建议。

**预期收益**：把分析升级为"教你怎么调 moveset"，而不仅是"告诉你当前配置差什么"。

---

### 10. 核心对（Core）打分完全忽略招式联动

**现状定位**：`analysis.js:497-507` 的 `scoreCoreEntry` 只统计 type-level 的 patch/shared weakness/coverage 指标。`analysis.js:566` 的 `buildCoreEntry` 也只合并 offensiveTypes，不看招式之间的协同。

**为什么是问题**：很多真正的核心来自招式互补而不是属性互补：

- Rillaboom + Urshifu-SS：Grassy Terrain 让 Grassy Glide 先制 + 伤害 +30%。
- Torkoal + Venusaur：天气起源 + Chlorophyll 速度双倍 + 日光束。
- Amoonguss + Cresselia：蘑菇孢子 + Light Screen/Reflect 高生存。
- Koraidon + Flutter Mane：晴天 + 高攻速 + 太阳之火。

这些协同工具现在完全看不见。

**建议做法**：`static/app/synergy-combos.js` 新建一张已知组合表：`[{ members: ["rillaboom","urshifurapidstrike"], bonus: 2.5, reason: "grassyterrain+grassyglide+priority" }, ...]`。`buildCoreEntry` 里加一笔 `comboBonus = lookupSynergyBonus(pair)`。可以初期从 VGC 顶级队伍手动标注 30~50 条。

**预期收益**：核心分析从"避免双死点"升级为"发现真正的双人打法"。
...
### 19. 推荐理由模板过于简短，缺少可解释性

**现状定位**：`recommendation-scoring/entry.js:12-38` 的 `buildReasons` 返回的是 `["覆盖 X 威胁", "速度 XX"]` 这种短语，并且各 reason 之间用 " / " 简单拼接。

**为什么是问题**：推荐系统最重要的信任来自"可解释"。当前卡片上看到的理由是"高贯输 / 速度 170 / 有威吓"，但用户问"为什么它比上一个高？"时工具无法回答。

**建议做法**：把 reasons 升级成结构化对象 `{ kind, weight, detail, delta }`，UI 渲染时按 weight 倒序展示，并在卡片下端给出"对比次选"按钮，点开后展现两者在每个 breakdown 项的差值。让"为什么是它"变成可见的数字差。
...
**现状定位**：`damage-workspace.js` 只做 `attacker ↔ defender` 单对计算。

**为什么是问题**：组队和调配过程中最常见的问题是"我的 Flutter Mane 能一发带走对面几只"。当前要一个一个换 defender 查，效率极低。

**建议做法**：在"伤害工作台"加入"对全队扫描"按钮——固定 attacker，批量对 6 只对手跑一次伤害计算，输出矩阵：`Flutter Mane | Moonblast | Urshifu 2HKO 85% | Rillaboom 2HKO 62% | Amoonguss OHKO 31% ...`。反向的"被全队打我"矩阵也做一份。
...
**现状定位**：`render-matchup.js:197-226` 的 `leadCardMarkup`。

**为什么是问题**：用户看到"Incineroar + Flutter Mane 分 32.5"只知道这对先发分高，不知道回合 1 应该让谁干什么——威吓先、Fake Out 锁住对方谁、Flutter Mane 这回合是保护还是开打。

**建议做法**：先发卡片加"推荐开局动作"行：

- 回合 1：`Incineroar Fake Out → Amoonguss`，`Flutter Mane Moonblast → Urshifu`。
- 回合 2：`Incineroar Parting Shot → pivot in Rillaboom`。
  剧本生成从"角色 + 招式 + 对方最脆的目标"反推即可，哪怕粗糙也比纯数字有用。
...
### 33. 对手快速选择缺少"按属性 / 速度档 / 战术角色"的筛选

**现状定位**：`render-matchup.js:77-98` 的 `quickPickSpriteMarkup` 只提供"搜索框"过滤。

**为什么是问题**：当前配置库里如果有 200+ 配置，六只对手要一个个翻过去。而用户往往知道"我要补个水系威胁进对面"或"对面需要一个顺风 setter 压制我"。

**建议做法**：在搜索框旁边加一排属性芯片 + 速度档位 + 角色芯片（可多选），点击即可叠加过滤。激活时搜索输入框并存，可组合。
...
### 41. 配置库卡片没有"对比两个配置差异"视图

**现状定位**：`render.js:303-338` 的 `renderLibrary` 只列出每个配置独立展示。

**为什么是问题**：用户常同时保存"剑舞型 Garchomp"和"Scarf 型 Garchomp"两套配置，要比较点数差异只能肉眼扫读两张卡。

**建议做法**：配置库卡片加 checkbox（最多选 2），然后在页面顶部显示"对比视图"——左右两列并排呈现两套配置的差异字段高亮。
...
### 46. 队伍形状类可视化总览 `未开始（评估后暂缓）`

**当前现状**：2026/04/20 曾试做"18 属性轮"（外圈我方进攻最佳倍率 / 内圈我方承受最差倍率 + legend），上线后用户立刻反馈"作用不大"——已整体回滚 JS / CSS / i18n，分析页恢复纯 metric-card 总览。
...
**现状定位**：分析页 `analysis-overview` 仍是 metric-card 数字块。

**为什么还是问题**：数字块（"3 个弱点 / 5 个盲点 / 8 个强势对象"）不能让用户一眼看出队伍属于哪种 VGC 架构（Tailwind 猛攻 / TR 重甲 / 晴队 Rag / 雨队 Arc / 平衡 pivot 等）。

**重新定向后的建议做法**：

1. **以"联防组合"为第一视觉语言**：内置一张已知互补组合表（火/水/草、鬼/恶/超、龙/钢/妖、虫/草/地、电/地/飞、钢/妖/龙等），对当前队伍自动匹配，命中的组合以绿色高亮、缺一只的用橙色"差 XX"的方式呈现，完全缺失的置灰。点击组合跳到"加哪只能补齐"的候选列表（和 #9 潜在 learnset 联动）。
2. **Archetype 识别 chip**：根据 `team-roles.js` 的标签（fakeout + intimidate + pivot + tailwind + trickroom + redirection + weather setter）归一化出 "你这支像：Tailwind 猛攻 / TR 重甲 / 晴队 / 雨队 / 混合平衡" 的架构标签一行。
3. **六边形雷达图（保留想法）**：以 "速度控制 / 进攻压制 / 防御韧性 / 支援密度 / 角色完整度 / 天气-场地节奏" 六维呈现整体形状。但雷达必须和 metric-card 字段**互补而非重复**。
4. **显式不做**：单只 x 单属性轮盘、按属性排的柱状图——已验证对 Champions 场景边际价值太低。

**预期收益**：打开分析页 2 秒内就能回答两个问题——「这支队是什么架构？」「联防闭环在哪缺口？」——而不是盯着一堆红绿色块逐格解读。
...
- 二期评估后暂缓 1 项：`#46`（属性轮盘原型上线后被用户评估为作用不大；已整体回滚，重新定向为"联防组合"可视化，见 #46 当前现状段）
- 其中仍建议继续深化的部分完成项：`#2`、`#3`、`#4`、`#5`、`#12`、`#14`、`#18`、`#23`、`#39`、`#57`（中期 node-level diff）、`#66`、`#75`
