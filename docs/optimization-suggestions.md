...
> 审阅时间：2026/04/19。
>
> 实现进度更新：2026/05/01（默认预设构造调整：`poke_analysis-main/default_preset/vgcpastes.py` 的同物种取 3 套规则改成"差异最大化 + 偏新加权"——按 item=1 / ability=5 / nature=2 / moves=4 / points=2 的加权距离做 max-min 多样性贪心，并以 CSV 的 `Date Shared` 列计算 recency 权重在 tie-break 中给较新的配置加分；同步把 `dateShared` 写入 `paste_sets_champions_ma.json` 的每条 selected.source，前端配置卡尚未消费此字段，已在文末新章追加 5 条 UI 推荐。体验密度回写：新增 `static/css/experience-polish.css` 收敛间距、卡片密度、小屏队伍横向浏览和搜索高亮；新增 `static/app/search-utils.js`，配置库与对局搜索共用匹配排序/`<mark>` 高亮；扩展关键导入、导出、保存、载入与自动填充 toast；按反馈撤回 #21 Speed Race 常驻表，只保留基础速度；撤除 #74 Canvas 手绘分享图；新增 `static/css/theme.css` 统一配色、排版和间距，新增 `static/css/motion.css` 统一微交互，并为主视图切换接入 View Transition）。2026/04/30（六期新手上手回写：#42、#45、#70、#71 已落地；五期体验增强回写：#27、#28、#47 已落地；#24 增加批量扫描筛选；推荐页新增候选对比与分析页焦点来源提示）。2026/04/26（三期核心回写：#24、#30 已落地；分析增强回写：#9、#10、#19 已落地；四期效率增强回写：#33、#41、#46 已落地，详见对应条目现状段）。2026/04/20 的二期核心回写：#37、#44、#57、#68 已落地。一期已落地 22 项：#1、#2、#3、#4、#5、#8、#11、#12、#16、#18、#20、#22、#23、#29、#31、#32、#34、#35、#36、#39、#40、#64；#14、#66、#75 仍为"部分完成"。另按用户反馈撤除了一批战况 UI toggle，详见 #1/#3/#4/#5 的"当前现状"段）。
>
...
9. 数据层、持久化与错误处理（6 条）
10. 工作流与功能扩展（6 条）
11. UI 增量补强（5 条，2026/05/01 新增）

总计 **80 个**可执行优化点（远超题目要求的 40 个），其中一期核心已完成 11 项，本文对应条目已标注状态并回写现状。读者可以先看"高价值 20 条"列表（文末附录 A）。
...
### 7. `team-roles.js` 的职能映射只给单一 Primary Role `已完成（解释性 v1）`

**当前现状**：`team-roles.js` 已拆为 `team-role-analysis.js / team-role-primary.js / team-role-metrics.js / team-role-rules.js / team-role-explanations.js`。`analyzePokemonRoles(config)` 现在输出 7 类主职能、可见副职能、技能格映射、技能格质量、道具影响、多职能评分与多职能判断；旧 `getStructureRoles(config)[0]` 仍保留给队伍结构与旧调用点兼容。分析页 Roles 面板展示单体职能卡，推荐卡、当前队伍卡、配置库卡和 VGCPastes 团队卡共用紧凑职能摘要。

**刻意不做**：`externalspeed / timingdependent / coverdependent` 这类条件风险不在 UI、tooltip 或推荐摘要中展示；它们只作为内部多职能评分输入，避免用户把"成立条件"误读成实际职能。
...
**剩余建议**：如果后续继续深化，可把角色缺位提示改成"弱支持 / 强支持 / 过剩"三档；阈值也可以继续按 ruleset 分组。当前版本先保持 Champions M-A 语境下的启发式，不引入 meta-aware 热门威胁识别。

**当前收益**：分析页和卡片摘要已经能同时展现主副角色、技能格质量和道具影响；UI 使用"先制 / 多职能 / 职能较单一"等直白文案，推荐解释不再只靠一个旧结构标签。
...
### 21. 伤害计算结果的速度对比只看单值，不做"race" `已撤回（2026/05/01）`

**当前现状**：原先的 `Speed Race` 微表格已按用户反馈撤除。伤害工作台现在只保留基础速度结论（谁更快 / 是否同速），不再展开我方顺风、对方顺风、双方顺风和 TR 反转这些用户能快速心算的场景。

**撤回原因**：这些场况的速度倍数规则足够简单，展开成 5 格表会降低伤害工作台的信息密度；基础速度给出后，用户自行心算更快。

**现状定位**：`render-damage.js:285-307` 的 `speedVerdictMarkup` 仅比较 `attacker.speed` 与 `defender.speed`。

**为什么暂不采用**：VGC 伤害计算时确实会关心"在什么场况下我能先手"，但顺风 / 双方顺风 / TR 的心算成本低，常驻表格带来的视觉负担高于收益。

**后续建议**：除非未来能和真实场况输入或速度线定位联动，否则不要恢复常驻 race 表。
...
### 24. 伤害计算不支持批量"对全队 / 对全对手" `已完成（三期，五期增强）`

**当前现状**：`damage-workspace.js` 已新增 `scanAttackerAgainstTargets()` 与 `scanAttackersIntoDefender()` 两条批量接口；`render-damage.js` 现已接入"我打对面全队 / 对面全队打当前我方"两个入口，结果以矩阵卡片呈现，点击任一单元格可直接跳回对应单对位继续细查。前者固定当前 attacker，逐招扫描当前 `opponentTeam`；后者固定当前 defender，找出对面每只最痛的招式与 KO 结论。**2026/04/30 五期增强**：`damage-scan-view.js` 增加 `全部 / OHKO / 2HKO / 最高伤害` 四个筛选 chip，只改变展示层，不改原始计算结果。

**剩余问题**：当前批量扫描仍基于 worker 的逐对串发，不是一次性矩阵计算；结果里还没有"导出 CSV / 复制表格"。
...
### 27. 场效（风格：独立场 / 己方场 / 对方场）字段没有分组视觉 `已完成（五期）`

**当前现状**：伤害工作台的己方场侧、对方场侧与独立场效果已按语义分组渲染为折叠区：屏障、保护 / 支援、光环 / 伙伴加成、钉子 / 持续效果、场地残留、全局能力、灾祸能力。每组继续复用原 checkbox 语义，字段名和计算入口不变。

**当前收益**：场效从大面积平铺开关变成可扫读的功能组，用户找 Tailwind、Light Screen、Helping Hand、Ruin 等高频项的成本明显下降。
...
### 28. 暴击、能力降低层级的交互组件不直观 `已完成（五期）`

**当前现状**：`boostGridMarkup` 已从 `<select>` 改成 chip 组，固定展示 `-6 -4 -2 0 +1 +2 +4 +6`，并新增当前侧"能力归零"以及 `Atk +2 / SpA +2 / Spe +2` 三个常用 preset。点击后会立即刷新伤害结果并清空过期批量扫描。

**当前收益**：调剑舞、诡计、速度强化等常见 VGC 场景时不再需要下拉滚动，当前 stage 也能一眼看出。
...
### 34.1 全局视觉系统缺少统一 token `已完成（2026/05/01）`

**当前现状**：新增 `static/css/theme.css`，集中定义深蓝主色、冷青辅助、暖橙强调、中性色表面、文字、边框、阴影、8px 圆角、8px 间距网格和 1.25 modular type scale；新增 `static/css/motion.css` 承载卡片入场、按钮过渡、View Transition 和 reduced-motion 规则。页面入口已引入主题层，并同步更新 `theme-color`。

**2026/05/01 体验密度补强**：新增 `static/css/experience-polish.css` 作为最后加载的体验修饰层，集中收紧 hero、主视图 tab、panel、card、chip 和 action row 的间距，给配置库/对局搜索命中统一 `<mark>` 视觉，并在移动端把当前队伍改为横向扫读，减少六张卡片把主工作区整体顶下去的问题。

**你在 UI 上怎么感知**：首屏、侧栏、主视图 tab、分析卡片、对局矩阵和伤害工作台都改为更克制的 dashboard 视觉；按钮 hover/active、卡片入场和主视图切换都有 400ms 内的轻量动效；`prefers-reduced-motion` 用户会自动降级到无动效。

**后续建议**：现有 CSS 文件历史较长，部分组件仍有局部硬编码颜色。后续改具体组件时优先把颜色、间距和阴影迁回 token；`experience-polish.css` 只承载体验收敛，不继续塞业务组件样式。
...
### 38. 搜索输入没有防抖，且不高亮匹配 `已完成（2026/05/01）`

**当前现状**：配置库搜索和对局快速选择搜索已经共用 `static/app/search-utils.js`，按物种名 / 配置标签 / 道具与特性 / 属性 / 招式分权重排序，并对命中片段输出 `<mark>` 高亮。配置库搜索仍保留原有 debounce；对局搜索保留 150ms debounce。搜索态下物种浏览器和配置卡都会显示命中的字段，用户能知道是名字、招式还是配置备注触发了结果。
...
**当前收益**：搜索不再只是“过滤出一堆卡片”，而是能直接告诉用户为什么命中；配置库和对局选择的搜索行为也不再各写一套。
...
- **2026/05/01 补强**：新增 `announceStatus(key, params, {toastType})`，导入/导出配置库、导入/导出队伍、导出/导入全部状态、清空配置库、载入预设、应用 starter、保存/载入/删除对面队伍、自动填充对手等关键路径已同步状态栏和 toast；`.toast-stack` 增加最大高度约束，避免极端堆叠撑出视口。

**剩余问题**：

- 仍有少量低风险内部状态（如撤销/重做、普通筛选切换）只更新 `#status-text`，避免 toast 噪音；
- Toast 目前只有最大高度约束，还没有“最多 3 条 + 溢出计数”的完整堆叠管理。

**后续建议**：

1. Toast stack 加最多 3 条常驻，溢出进折叠计数；
2. 错误 toast 加"复制详情"按钮，减少排障成本。
...
### 42. 导入/导出全都集中在一个 `<details>` 面板，路径被折叠 `已完成（六期）`

**当前现状**：主工作区顶部新增 `quick-start-panel`。当配置库为空或当前队伍未满 6 只时，会显示"快速开始"区，直接提供载入默认预设、粘贴整队、粘贴配置库、查看分析四个动作；同时展示 starter 模板卡片。原导入面板仍保留，但"清空配置库"已移动到高级操作折叠区，并加入二次确认。空输入点击导入不再隐式清空配置库，只会提示用户走明确的清空动作。

**当前收益**：首次访问不再需要先展开隐藏面板找"载入默认预设"。破坏性操作也从主路径移出，降低误点成本。
...
### 45. 队伍抗性矩阵用 CSS Grid + 数字文本，不够直观 `已完成（六期）`

**当前现状**：Coverage 的防守/进攻矩阵 cell 已加入连续强度编码：按倍率写入 `--matrix-cell-alpha` 与 `--matrix-cell-ring`，让 `0x / 0.5x / 2x / 4x` 的视觉差异更明显。每格仍保留倍率文本，并新增"免疫 / 抗 / 弱 / 强打 / 低效 / 普通"这类文字标记，避免只靠红绿判断。

**当前收益**：分析页第一屏更容易扫出"谁在扛关键弱点"和"谁负责打点"，窄屏下也能通过成员名 + 标记读懂单格含义。
...
### 47. 速度线视图是时间线结构，但没有比例感 `已完成（五期）`

**当前现状**：速度线渲染已改为按相邻速度差计算行距，速度差越大，行距越明显；同时加上 100 / 135 / 150 / 170 / 200 / 255 / 300 / 400 等 benchmark 参考线。为了避免 199/200 这类相邻速度互相压住，行距带有最小/最大夹取，不做不可读的纯绝对定位。

**当前收益**：速度线终于能体现"差 1 点"与"差 80 点"的视觉差异，同时仍保持多配置、多变体速度条的可读性。
...
### 49. 分析维度切换（Coverage / Roles / Cores）不保留上下文 `部分完成（五期增强）`

**当前现状**：当前页面结构已经是三个 panel 常驻 DOM，通过 `.active` 切换显示；五期补强了分析页跳推荐的焦点来源提示，用户从 Coverage 的"按 XX 补位推荐"进入推荐页后，会看到"当前焦点来自分析页补位入口"。推荐页还新增最多 2 个候选的分项对比，减少在分析 / 推荐之间来回切换。

**剩余问题**：展开/折叠状态还没有形成独立的 `analysisUi` 持久化结构；如果后续加入更多可折叠块，还需要统一保存这些 UI 状态。
...
### 56. 移动端布局：六只队伍卡片在窄屏挤压 `部分完成（2026/05/01）`

**当前现状**：`static/css/experience-polish.css` 已对 768px 以下布局做第一轮紧凑化：当前队伍 `.team-grid` 从竖向堆叠改为横向扫读卡片，侧栏动作按钮改为两列，tab / chip / card 间距同步收紧。这样用户在手机上先横向查看 6 只队伍成员，再继续向下进入工作区，不再被 6 张完整卡片连续顶开。
...
**剩余问题**：尚未做真正的底部抽屉或 workspace/team 互斥 tabs；本轮选择低风险 CSS 收敛，避免引入新的移动端状态机。
...
### 70. 无"队伍模板"一键启动 `已完成（六期）`

**当前现状**：新增 `static/app/starter-templates.js`，基于现有 `config-default.txt` 里的真实配置提供 4 个 starter：平衡起手、雨天进攻、晴天进攻、空间队。点击模板会先确保默认配置库已加载，再从库内按物种匹配 6 条配置写入当前队伍；若缺配置，会明确列出缺失 species id，不伪造成功。

**当前收益**：新用户可以从"空仓库"一步进入可分析的 6 人队，不必先理解配置库、队伍、推荐之间的关系。
...
**当前现状**：快速开始区内置轻量步骤条：配置库已就绪、当前队伍满 6 只、查看分析结果。步骤根据真实 `state.library / state.team / state.activeView` 自动更新，不额外引入可跳过的 modal 状态。

**当前收益**：新手能看到下一步该做什么，且不会被弹窗打断；完成 6 人队后仍可直接跳到分析页。
...
### 74. 无"数据导出为图片" `已撤除（2026/05/01）`

**当前现状**：原先的 `static/app/share-image.js` Canvas 手绘方案已撤除，队伍分析、对局分析、伤害工作台里的"导出分享图"按钮也已移除。Service Worker 不再 precache 该模块，i18n 中的分享图文案同步删除。

**撤除原因**：Canvas 手绘和真实 DOM 视觉体系脱节，信息密度、字体、换行和可维护性都不理想；继续修补会形成第二套渲染系统，长期成本高于收益。

**当前收益**：页面不再暴露质量不足的分享入口，也少了一条图片资源加载和 Canvas 导出失败路径。

**现状定位**：分析页内容都是 DOM，截图要靠浏览器。

**为什么是问题**：玩家常把队伍分析或伤害计算结果贴到 Discord / 微博。当前只能手动截图，分享率低。

**建议做法**：如果未来重做，优先选择 DOM/打印友好方案：给核心 panel 增加 `@media print` 和 `.share-surface` 样式，再评估 `html-to-image` 或 `dom-to-image-more`。重做前不要恢复 Canvas 手绘路径。
...
## 十一、UI 增量补强（2026/05/01 新增）

> 本节是结合最近一次默认预设构造逻辑迭代（同物种取差异最大 3 套配置 + 加权偏新）后顺手梳理的 UI 推荐。重点放在"已有功能信号没用足"的方向，避免和前面已落地的项目重复。

### 76. 配置库的"差异预览"在卡片上没暴露 `未开始`

**现状定位**：默认预设构造侧（`poke_analysis-main/default_preset/vgcpastes.py`）目前会为同一只宝可梦保留差异最大的 3 套配置（道具/特性/性格/招式 集合 + 66 点分布的加权差异），并按"分享日期越新权重越高"做 tie-breaker。但前端 `render.js` 渲染配置卡时并没有任何视觉标记说明"这些配置是被刻意挑成 spread 出来的不同流派"。

**为什么是问题**：用户看到 Incineroar 同时有 `Sitrus Berry / Adamant / 引导壳`、`Choice Scarf / Jolly / 抢节奏`、`Assault Vest / Careful / 特防壳` 三套时，如果没有提示，很可能误以为只是历史积累，倾向于只用排在最上面的那套。差异维度（道具改、性格改、招式改）的可读性等于 0。

**建议做法**：

1. 卡片右上角加一个 `差异` chip：列出与同物种其他默认配置之间真正变化的字段（例：`变：道具 / 招式` `变：特性 / 性格`），最多 3 项 chip。
2. hover 时弹出"差异雷达"小卡：5 维（道具 / 特性 / 性格 / 招式 Jaccard / 点数 L1）放射图，让用户一眼看出"这套和兄弟配置的差距集中在哪"。
3. 当鼠标在卡片 A 上时，同物种其他默认配置卡片的差异维度应高亮（联动），帮用户对照阅读。
4. 后端在 `paste_sets_champions_ma.json` 的 `selected[]` 上额外写入 `diversitySignals: ["item","ability","moves"]` 列表，前端不必重算。

**预期收益**：让"为什么默认给我推这三套"从隐式策略变成显式信号；同时引导用户在调点之前就先思考流派切换，而不是只把第一张拿来改。

---

### 77. 默认预设的"分享日期"在 UI 完全隐形 `部分完成`

**现状定位**：`vgcpastes.py` 现在会读取 CSV 的 `Date Shared` 列并写入每条 selected 配置的 `source.dateShared`，并按日期参与差异选择的 tie-breaker。前端 `render.js` 渲染配置卡时只用 `note` 显示 `VGCPastes PCxxx`，没有露出日期。

**当前现状**：默认预设导入时会从 `static/paste_sets_champions_ma.json` 按 teamId / 物种 / 道具 / 特性 / 性格 / 招式补回 `source` 元数据；配置库卡片和队伍卡片会显示 `YYYY-MM-DD PCxxx` 日期标签，并对近 30 天来源着绿色强调。尚未加入按分享日期排序 / 筛选。

**为什么是问题**：

1. M-A 赛季 meta 在 4 个月内变化明显，3 月底的"假面玛娘+鬼勾"流派可能在 4 月被 Mega Heracross 改变；用户没法判断默认配置的时效。
2. 工具内已有"加权偏新"的事实（在 Python 侧），但用户感知不到这件事，自然也不知道为什么 4 月的配置会优先出现在前面。

**建议做法**：

1. 配置卡 footer 显示 `2026-04-15 PCxxx`；近 30 天内打 "🆕 新" badge，60 天以上打 "🕒 旧" 暗化。
2. 配置库支持按"分享日期"排序 / 筛选；推荐页可加一个 "只看 60 天内 meta" 的开关。
3. `paste_sets_champions_ma.json` 的 `info` 块补 `dateRange: { min, max }`，让前端不必再扫一遍 selected 算时效摘要。

**预期收益**：把后端已经做出的"偏新"决策从黑盒改成可见信号；老配置和新配置的取舍权交还给用户，而不是在 Python 里默默生效。

---

### 78. 导出 / 导入弹窗仍是 `<details>` + `<textarea>`，缺乏分屏对比 `未开始`

**现状定位**：`#42`（六期已落地）已把"快速开始"做成显式入口，但完整的导入/导出 UI 仍然是 textarea + 摘要文本。粘贴整个 6 人队伍 Showdown 文本做"局部覆盖单只"或"对比当前队伍 vs 粘贴版本"的需求没有 UI。

**为什么是问题**：玩家从 Discord / Twitter 拷贝来一只精灵想覆盖当前队伍里的同名位，现在只能整段粘贴，"覆盖 / 追加"二选一；没法做"我只要导入这只精灵，其他保留"。

**建议做法**：

1. 导入 modal 改两栏：左边 textarea + 解析 preview，右边显示 6 张候选卡片，每张可单独勾选"导入到当前队伍 / 仅导入到配置库 / 跳过"。
2. 解析 preview 直接复用 `parseShowdownLibrary().feedback[]`，warning 显式指向卡片。
3. 导入时如果检测到当前队伍已有同物种配置，给出"覆盖现有 / 保留两套 / 跳过"三选一。

**预期收益**：导入流程从"全量替换"转向"逐只决策"，这是 VGC 玩家在调点阶段最常见的真实操作。也能减少 `#36`（撤销）兜底的负担。

---

### 79. 配置卡的 hover 信息密度浪费 `未开始`

**现状定位**：当前配置库卡片 hover 没有任何额外信息；只有点击进编辑器才能看完整 moves / points / nature。但屏幕上 96px 的卡片已经预留了大量留白。

**为什么是问题**：用户在配置库扫读阶段最想看到的是：六维属性条、四招缩略、关键 breakpoints（`+1 / Scarf / Tailwind` 后的速度档），而不是只看"图标 + 道具 + 特性"。

**建议做法**：

1. hover 时卡片右下角浮一个 200×120 的小预览：六维堆叠条（按当前 66 点配置 + 性格修正算实际值）、四招 pill 行、`Spe @ +1 / @ Scarf / @ Tailwind` 三档。
2. 触摸设备上保留 long-press（500ms）触发同样浮层；配合 `#53` 的整体 `title -> popover` 替换一起做。
3. 浮层内容由 `team-config.js` 的现有 hydrate 结果生成，不需要新计算。

**预期收益**：扫库速度提升一倍以上；不必逐个点开编辑器才能比较两只候选的真实属性。

---

### 80. 主视图切换的"在做什么"指示缺失 `部分完成`

**现状定位**：`#37`（已完成）让数字键 `1-6` 切主视图很顺手，但切到对局分析或推荐页时，新页面的"上下文（当前 team / 当前 opponentTeam / 焦点属性）"是隐式的——用户切过去要先扫一眼侧栏才能确认"这是基于哪支队伍在分析"。

**当前现状**：主功能标签下已新增上下文条，持续显示当前视图、队伍数量、对面数量、队伍架构和推荐焦点；点击队伍 / 对面 / 架构 / 焦点标签可跳转到对应入口。尚未加入视图切换时的 200ms 高亮动画，也没有队伍命名 modal。

**为什么是问题**：在快速循环 `1 → 5 → 3 → 4` 时，用户每切一次都要重新建立心智上下文，工作记忆负担过大。

**建议做法**：

1. 顶栏（hero 下方）固定一行薄薄的"上下文条"：`队伍：未命名（6 / 6） · 对面：3/6 · 焦点：水` 这样的 inline 标签。
2. 视图切换时该条带 200ms 高亮一次，提示"上下文跟过来了"。
3. 上下文条上的标签点开可直接跳到对应编辑入口（点"队伍"打开队伍命名 modal、点"对面"切到对局分析）。

**预期收益**：把"我在哪、在做什么"的提示成本从"每次切视图后扫侧栏"压到 0；和命令面板（`#44`）的"行动上下文"形成互补。

---

## 十二、2026/05/02 迭代落地记录

这一轮以"用户实际反馈驱动"，把推荐打分、UI 视觉、搜索体感、IME、配置选择等一批小痛点收齐。下面按主题汇总，方便后续回溯改动来源。

### 12.1 推荐 / 滑块算法

- **滑块自动默认值彻底重写** `已完成`：`getRecommendationPairingCrossoverBias` 之前用解析公式找 [0,1] / [1,2] flip share，floorPenalty 不入参导致预测和可见排名脱节，反复算出 0 / 1 / 12 / 54 / 100。最终改成"按真实 `effectiveScore = (team*(1-s)+pair*s)*floorPenalty` 模拟，从 slider=99 一格一格往下扫，rec #1 第一次发生变化时返回该格"，并取**较小**的边界值。配套修了上层 `Number.isFinite(Number(autoPresetBias))` 误把 `null` 当 0 的判空 bug、`getRecommendationScoreMix` 的 `presetBias` 同样判空 bug。
- **`只看 Mega` 切换接到 template 候选** `已完成`：`recommendations.js` 的 `templateRecommendations` 之前没用 `state.recommendMegaOnly` 过滤，切换后只对 library 子集生效。修复后传 `megaOnly` 进 `recommendConfigs` 选项，library 和 template 两条管线都按 `isMegaConfig` 过滤。
- **`chooseBestRecommendationBySpecies` 加入 role-fit 平局裁决** `已完成`：当同种宝可梦的两套 config `recommendationScore` 差距 ≤ MAX×5%（≈2 分）时，改用 `synergy + counterChain` 做主键，让"补缺失职能 / 反制链强"的配置在近似情形胜出，避免被 12% 上限的 synergy 在综合分里冲淡。综合分明显更优的不会被错过。

### 12.2 UI / 视觉

- **加分扣分块视觉重做 + counter 翻译** `已完成`：`render-recommendations.js` 的 `renderInsightBlock` 现在把负向 toneClass 提升到块容器（`recommend-insight-block-negative`）；`analysis.css` 给两块加了左侧色条（绿/红）、轻底色、字号收紧，负向块整体红色调。zh 文案 `recommend.reason.counterChain` 中"counter" 替换为"克制者"。
- **上下文 chip 撑满整行修复** `已完成`：根因是 `base.css` 的 `select, input, textarea, button { width: 100% }` 把 `<button>` 类型的 chip 撑满容器。`shell.css` 的 `.workspace-context-chip` 加 `width: auto` 覆盖。
- **单体职能分析 3×2 布局** `已完成`：`.single-role-grid` 从 `auto-fit minmax(240px,1fr)` 改成 `repeat(3, 1fr)`，配 `@media (max-width: 960px)` → 2 列、`(max-width: 600px)` → 1 列的响应式回退。
- **快速开始永久关闭** `已完成`：加"不再显示"按钮，新增持久化字段 `state.quickStartDismissed`，关一次后该浏览器永远不再弹。`render-quick-start.js` + `persistence.js` + `main.js` + i18n 双语。
- **VGCPastes 过时提示删除** `已完成`：`vgcpastes.copy`（"按分享日期倒序展示最新 30 支真实赛队…"）已不再准确，从 picker 渲染中移除（i18n key 暂留以防其他位置引用）。

### 12.3 搜索性能 / IME

- **搜索文本缓存（VGCPastes）** `已完成`：`vgcpastes-search.js` 加 `WeakMap` 缓存，按 team 对象+language 做键，避免每次按键都对 374 队全量拼接 `teamSearchText`。
- **每队卡片 markup 缓存** `已完成`：`render-vgcpastes-picker.js` 加 `TEAM_CARD_CACHE` (WeakMap)，按 team 对象 + `language|iconScheme` 做版本签名，sprite/tooltip/角色标签只拼一次，过滤刷新只拼字符串拼接。
- **搜索框 IME 修复** `已完成`：新增 `attachImeAwareSearchInput(el, runHandler)` 工具——监听 `compositionstart/compositionend` 维持组合状态，`input` 事件期间 `event.isComposing === true` 时不触发 debounce/重渲染，组合结束才触发。应用到 `library-search`、`matchup-search`、`saved-team-search`。VGCPastes 用事件委托，在 `input` 里加 `event.isComposing` 守卫，并补委托的 `compositionend` 监听器。
- **`saved-team-search` 补 200ms 防抖** `已完成`：之前每次 keystroke 直接 `renderSavedTeams`，已改成 debounce + IME-safe 包装。

### 12.4 文档 / Memory

- **§12 单体职能完整分类（110 条）已存档** `已完成`：用户提供的 9 大类、约 110 条职能写入 `docs/future-todo-reference.md` §12，并附 Phase A/B/C/D 分期落地建议（先做 10–20 条高频补全，再细分输出 / 防御 / 控速 / 干扰，最后才做反制位与综合类）。
- **memory: 推荐滑块算法约定** `已完成`：把"必须用模拟+floorPenalty / 取较小边界 / 不能加 topN / 不能用 `Number.isFinite(Number(null))`" 这套教训写进 `~/.claude/projects/-home-zonazcy-Projects-poke-type/memory/feedback_recommendation_slider.md`，避免下次再走弯路。

### 12.5 SW 缓存版本

本次连续递增到 `poke-type-v50-20260502`。每次涉及 `static/app/*.js` / `static/css/*.css` / `index.html` / `sw.js` 改动都同步 bump，避免老版 SW 仍在 cache-first 命中旧资源。
...
- 六期新增落地 4 项：`#42`（快速开始与高级清空）、`#45`（Coverage 矩阵强度编码）、`#70`（starter 队伍模板）、`#71`（轻量组队引导）
